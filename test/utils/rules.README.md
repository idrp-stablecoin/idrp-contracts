...
| `rules.confiscate.json` | `Confiscate` (6) | 1 (all 4 roles) | Single top tier, any amount — deliberately structured so `amount` cannot select a weaker tier (same reasoning as Mint/Burn's tier-immunity: understating `amount` only under-confiscates). This file governs *who* may authorise a seizure. It says nothing about *where* seized funds go — that destination is a separate, admin-owned, 48h-timelocked storage slot on the token (`IDRP.confiscationWallet`, set via `scheduleConfiscationWallet` / `applyConfiscationWallet`), deliberately independent of this quorum. |
