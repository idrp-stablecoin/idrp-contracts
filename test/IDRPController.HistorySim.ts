...
describe("IDRPController - pre/post upgrade history simulation", function () {
  ...
  /**
   * Slot of `usedSignatures` on the TRON lineage — 254, not 3. This lineage
   * carries the legacy AccessControl gap ahead of it, so the same variable sits
   * at a completely different slot. Verified against storageLayout.
   */
  const USED_SIGNATURES_SLOT = 254n;
