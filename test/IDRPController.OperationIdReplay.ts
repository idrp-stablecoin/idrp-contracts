...
// Reproduces the Kaia mainnet double mint of 2026-09-17: the same
// operationIdentifier executed twice because the replay key was the EIP-712
// digest, and the digest includes the deadline. Two deadlines 608 s apart gave
// two digests, so the second mint sailed past usedSignatures.
describe("IDRPController - operationIdentifier replay", function () {
  ...
  // The incident, minus the amount: one identifier, one quorum, two deadlines.
  it("rejects a second mint under the same identifier with a fresh deadline", async function () {
    ...
    const identifier = "5baf69c2-58a1-41f3-a3ff-84bdc407e873";
    const amount = hre.ethers.parseUnits("354960000114", 6);
