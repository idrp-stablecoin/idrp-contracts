...
// TEST FIXTURE — layout-exact replica of the deployed Tron mainnet IDRP token. Do not deploy.
//
// CONSTRUCTED, not identified. The deployed token implementation is unverified on
// Tronscan and its exact source is not recoverable from this repo: every later commit
// with the right field order uses the OZ 5 `_update` hook and will not compile under
// OZ 4. So this is built to match the OBSERVED layout, which is what an upgrade depends
// on, and nothing more is claimed.
//
// Built from contracts/IDRP.sol at ac5b117, with two changes derived from measurement:
//   1. stock `UUPSUpgradeable` instead of `TronUUPSUpgradeable`. Stock carries a
//      `__gap[50]`, which moves the app variables from 454 to 504 — matching live — and
//      the deployed bytecode contains no TronUUPS marker, so it cannot be TronUUPS-based.
//   2. `depositoryWallet` declared before `maxSupply`. ac5b117 has them the other way,
//      which put them at 505/506 swapped relative to the live proxy.
//
// Verified against Tron mainnet: frozen 504, depositoryWallet 505, maxSupply 506,
// upgrader 507, upgradeScheduledAt 508, scheduledImplementation 509, sanctionsList 510.
...
contract MainnetReplicaV2Token is
    Initializable,
    ERC20Upgradeable,
