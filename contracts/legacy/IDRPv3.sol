...
//
// The v3 token as it stood before `admin` and `upgrader` moved to a delayed
// two-step handover. It is the upgrade-from source for tests and fork
// rehearsals of that change.
//
// The only edits applied vs that source:
//   - contract IDRP             → contract IDRPv3      (avoid name collision)
//   - interface ISanctionsList  → ISanctionsListV3     (avoid name collision)
//
    ...
    UUPSUpgradeable
{
    ...
    function _update(
        ...
        if (from != address(0) && to != address(0)) {
            ...
            if (list != address(0)) {
                if (ISanctionsListV3(list).isSanctioned(from)) {
                    revert SanctionedSender(from);
                }
                if (ISanctionsListV3(list).isSanctioned(to)) {
