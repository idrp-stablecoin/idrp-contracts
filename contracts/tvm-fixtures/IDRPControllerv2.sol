...
//
// Source: contracts/legacy/IDRPControllerv2.sol
...
// ─────────────────────────────────────────────────────────────────────────────
// LEGACY v2 SNAPSHOT — DO NOT EDIT THIS FILE.
//
// This is a byte-for-byte preserved copy of the IDRPController v2 implementation
// that is currently deployed to mainnet (Ethereum, BSC, Polygon, Kaia) and Tron.
// Source-of-truth: deployment/logs/contracts/v2.IDRPController.sol (Etherscan).
//
// It exists in contracts/ ONLY so that scripts/v1-to-v2/ can compile and
// deploy it onto testnets that are still stuck at v1 (Base Sepolia controller).
// After those testnets reach v2, the v2→v3 migration uses the production
// contracts/IDRPController.sol just like every mainnet does.
//
// The only edits applied vs the verified source:
//   - contract IDRPController → contract IDRPControllerv2
//   - interface IIDRP         → interface IIDRPLegacy
//
// Function bodies and storage layout are UNCHANGED. If you need to update
// the legacy v2 path, refresh from deployment/logs/contracts/v2.IDRPController.sol
// and re-apply only those two renames.
// ─────────────────────────────────────────────────────────────────────────────
...
contract IDRPControllerv2Tvm is
    Initializable,
