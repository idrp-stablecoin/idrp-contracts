...
//
// This is a byte-for-byte preserved copy of the IDRP v2 implementation that
// is currently deployed to mainnet (Ethereum, BSC, Polygon, Kaia) and Tron.
// Source-of-truth: deployment/logs/contracts/v2.IDRP.sol (Etherscan-verified).
//
// It exists in contracts/ ONLY so that scripts/v1-to-v2/ can compile and
// deploy it onto testnets that are still stuck at v1 (Kaia Kairos IDRP).
// After those testnets reach v2, the v2→v3 migration uses the production
// contracts/IDRP.sol just like every mainnet does.
//
// The only edits applied vs the verified source:
//   - contract IDRP   → contract IDRPv2     (avoid name collision with v3 IDRP)
//   - interface ISanctionsList → ISanctionsListLegacy   (avoid name collision)
//
// Function bodies and storage layout are UNCHANGED. If you need to update
// the legacy v2 path, refresh from deployment/logs/contracts/v2.IDRP.sol
...
interface ISanctionsListLegacy {
    function isSanctioned(address addr) external view returns (bool);
}

contract IDRPv2Tvm is
    Initializable,
    ERC20Upgradeable,
    ...
    UUPSUpgradeable
{
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) {
            ...
            if (list != address(0)) {
                if (ISanctionsListLegacy(list).isSanctioned(from)) {
                    revert SanctionedSender(from);
                }
                if (ISanctionsListLegacy(list).isSanctioned(to)) {
