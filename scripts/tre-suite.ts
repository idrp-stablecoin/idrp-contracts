...
// Fixtures generated by scripts/tvm/make-fixtures.ts. They differ from the real
// sources ONLY in UPGRADE_DELAY and the contract name, so a rehearsal finishes in
// minutes instead of two days. V2 comes from contracts/legacy/IDRPv2.sol, proven
// byte-identical to the implementation live on Tron mainnet
// (scripts/verify-deployed-source.ts).
const V2 = "IDRPv2Tvm";
const V3 = "IDRPTvm";
...
async function main() {
  ...
  const v2 = await deploy(V2, [], "v2 token (deployed mainnet source)");
  const v3 = await deploy(V3, [], "v3 token");
  const v3b = await deploy(V3, [], "v3 token (second)");
  ...
  const preV2 = await deploy(V2, [], "v2 (pre-existing proxy)");
